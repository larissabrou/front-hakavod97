...
const newBlock = `{!isAuthenticated ? (
          /* ── NOT AUTHENTICATED: Instreet-style Side-by-Side ── */
          <div className="max-w-6xl mx-auto bg-transparent mb-20 mt-10">
            {error && (
            ...
            {showForgotPassword ? (
              /* ────── MOT DE PASSE OUBLIÉ ────── */
              <div className="max-w-md mx-auto bg-white p-8 md:p-12 rounded-none shadow-sm border border-neutral-200 animate-fade-in">
                {!forgotSent ? (
                  <form onSubmit={handleForgotPassword} className="text-left">
                    ...
                    <button
                      ...
                      className="w-full bg-[#ff671d] hover:bg-[#e55c19] text-white py-4 text-xs font-bold uppercase tracking-widest transition-all rounded-none flex items-center justify-center gap-2 cursor-pointer"
                    >
                  ...
                  <div className="text-center animate-fade-in flex flex-col items-center py-4">
                    ...
                    <div className="flex flex-col gap-3 w-full mt-4">
                      ...
                      <button
                        ...
                        className="w-full bg-[#ff671d] hover:bg-[#e55c19] text-white py-3.5 text-[10px] font-bold uppercase tracking-widest transition-all rounded-none cursor-pointer"
                      >
                        {c.back_to_login}
                      </button>
                    </div>
              ...
              <div className="max-w-md mx-auto bg-white p-8 md:p-12 rounded-none shadow-sm border border-neutral-200 animate-fade-in">
                <form onSubmit={handleVerifyOtp} className="space-y-5 text-left">
                  ...
                  <button
                    type="submit"
                    disabled={loginLoading}
                    className="w-full bg-[#ff671d] hover:bg-[#e55c19] text-white py-4 text-xs font-bold uppercase tracking-widest transition-all rounded-none flex items-center justify-center gap-2 mt-4 cursor-pointer"
                  >
              ...
              <div className="flex flex-col md:flex-row gap-6 md:gap-8">
                ...
                {/* LEFT COLUMN: GIRIŞ YAP (CONNEXION) */}
                <div className="w-full md:w-1/2 bg-white p-6 md:p-10 border border-neutral-200 rounded-none shadow-sm flex flex-col">
                  <h2 className="text-lg md:text-xl font-bold text-neutral-900 mb-6 uppercase tracking-wider border-b border-neutral-200 pb-4">
                    {locale === 'en' ? 'Sign In' : 'ÜYE GİRİŞİ (Connexion)'}
                  </h2>
                  ...
                  <form onSubmit={handleLogin} className="space-y-4">
                    ...
                    <button
                      type="submit"
                      disabled={loginLoading}
                      className="w-full bg-[#ff671d] hover:bg-[#e55c19] text-white py-3.5 text-xs font-bold uppercase tracking-widest transition-all rounded-none flex items-center justify-center gap-2 mt-6 cursor-pointer"
                    >
                      {loginLoading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : 'GİRİŞ YAP (Se connecter)'}
                    </button>
                  </form>
                ...
                {/* RIGHT COLUMN: ÜYE OL (CRÉER UN COMPTE) */}
                <div className="w-full md:w-1/2 bg-neutral-50 p-6 md:p-10 border border-neutral-200 rounded-none shadow-sm flex flex-col">
                  <h2 className="text-lg md:text-xl font-bold text-neutral-900 mb-6 uppercase tracking-wider border-b border-neutral-200 pb-4">
                    {locale === 'en' ? 'Register' : 'ÜYE OL (Créer un compte)'}
                  </h2>
                  ...
                  <form onSubmit={handleRegister} className="space-y-4">
                    ...
                    <div className="pt-2">
                      <label className="flex items-start gap-2 cursor-pointer">
                        <input type="checkbox" className="mt-1 accent-[#ff671d] w-3 h-3" required />
                    ...
                    <button
                      type="submit"
                      disabled={loginLoading}
                      className="w-full bg-[#ff671d] hover:bg-[#e55c19] text-white py-3.5 text-xs font-bold uppercase tracking-widest transition-all rounded-none flex items-center justify-center gap-2 mt-4 cursor-pointer"
                    >
                      {loginLoading ? <Loader2 className="w-4 h-4 animate-spin text-white" /> : (registerMethod === 'phone' ? (locale === 'en' ? 'Send OTP SMS' : 'Recevoir le code OTP') : 'ÜYE OL (Créer un compte)')}
                    </button>
                  </form>
                </div>
...
console.log("Successfully patched Account.jsx");
